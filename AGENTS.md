...
- Core Flutter code is in `lib/`, with BLE protocol definitions in `lib/connector/hamcore_protocol.dart` and BLE transport/state in `lib/connector/hamcore_connector.dart`.
...
- Discovery: scans for devices advertising the Nordic UART Service UUID; `HamCoreUuids.deviceNamePrefixes` (`HamCore-`) is for reference/display only, not a scan filter.
- Frames are capped at `maxFrameSize = 172` bytes; byte 0 is the command/response/push code. I/O is `HamCoreConnector.sendFrame` and `HamCoreConnector.receivedFrames`.
...
- Device info: `cmdAppStart` triggers `respCodeSelfInfo` with tx power, pubkey, lat/lon, telemetry flags, radio params, and node name (see offsets in `lib/connector/hamcore_connector.dart`).
...
- Group text packets (`PAYLOAD_TYPE_GRP_TXT`): payload is `[channel_hash (1)][MAC (2)][encrypted data...]`. Decrypted data layout is `[timestamp x4][txt_type][text...]` where text is `"sender: message"` (see upstream MeshCore firmware's `BaseChatMesh::sendGroupMessage`, which this format derives from). Sender identity is not in the payload; use `PUSH_CODE_LOG_RX_DATA` raw packet path bytes for origin hash when available.
