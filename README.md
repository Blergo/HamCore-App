...
5. **Security:** Replaced the repeater login password prompt with Guest Access / Admin Access buttons. Guest Access now works — the firmware grants it unconditionally via a dedicated request type (`ANON_REQ_TYPE_LOGIN_GUEST`) instead of a password check, so the app just asks for it directly. Admin Access stays disabled since the firmware doesn't implement admin auth yet (`ANON_REQ_TYPE_LOGIN_ADMIN` is wired but always rejected server-side for now). Routing controls (path mode, manage paths) remain available from the same dialog.
