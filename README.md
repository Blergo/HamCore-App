...
- Replaced the repeater login password prompt with two Guest Access / Admin Access buttons. Both are currently disabled (greyed out) since repeater authentication is broken on the firmware side; routing controls (path mode, manage paths) remain available from the same dialog.
- **[WIP]** Channel message "heard repeats" counting: fixed several bugs stemming from HamCore firmware sending channel payloads in plain text with a zeroed-out MAC field instead of encrypting them. Raw-packet repeat parsing now reads the correct byte offsets for the new format instead of failing every decrypt attempt; sending the same text twice in quick succession no longer gets miscounted as a heard repeat instead of a new message; the dedup hash used to match repeats is upgraded to an exact packet hash once available instead of staying on a fuzzy text/time match; and self-sent messages echoed back by a repeater are no longer dropped before they can be counted. Not yet verified against real repeater hardware.
