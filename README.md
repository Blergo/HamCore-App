...
2. **Channel Chat UI:** Hid the hop-count/"via" path tracing row on outgoing channel messages — it always showed a misleading "1 hop" (the repeat-count badge already covers this), and only ever reflected one arbitrary repeater's echo path when multiple may have repeated the message. Incoming messages are unaffected.
3. **Message Repeats Screen:** Long-pressing an outgoing channel message now opens a dedicated "Repeats" screen instead of "Path" — it lists every repeater that echoed the message back, each rendered with the same hop-timeline treatment as the incoming-message path screen, rather than picking one arbitrary "primary" path and demoting the rest to raw hex. Incoming messages are unaffected and still open the existing Path screen.
4. **Repeat Counting:** Fixed a bug where an identical-text message sent from a *different* companion device within the merge window got folded into your own outgoing message as a "heard repeat" instead of showing as its own message. The self-echo check was comparing the wrong side of the match (your own sent message's sender name, which is always your own name) instead of verifying the incoming copy actually claimed to be from you.
